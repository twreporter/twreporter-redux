...
describe('denormalize assets testing', () => {
  ...
  context('denormalize posts', () => {
    ...
    it('should handle multiple slugs', () => {
      expect(denormalizePosts(['post-slug-1', 'post-slug-2'], postEntities)).to.deep.equal([{
        slug: 'post-slug-1',
        relateds: ['post-slug-2', 'post-slug-3'],
      }, {
        slug: 'post-slug-2',
    ...
    it('shoulde return new object', () => {
      ...
      expect(post[0].relateds).to.not.equal(postEntities['post-slug-1'].relateds)
    })
  })
  context('denormalize topics', () => {
    ...
    it('should handle single slug', () => {
      expect(denormalizeTopics('topic-slug-1', topicEntities, postEntities)).to.deep.equal([{
        slug: 'topic-slug-1',
        relateds: [{
          slug: 'post-slug-4',
          topics: 'topic-slug-1',
    ...
    it('should handle multiple slugs', () => {
      expect(denormalizeTopics(['topic-slug-1', 'topic-slug-2'], topicEntities, postEntities)).to.deep.equal([{
        slug: 'topic-slug-1',
        relateds: [{
          slug: 'post-slug-4',
          topics: 'topic-slug-1',
    ...
    it('shoulde return new object', () => {
      const topic = denormalizeTopics(['topic-slug-1'], topicEntities, postEntities)
      expect(topic[0]).to.not.equal(topicEntities['topic-slug-1'])
      expect(topic[0].relateds).to.not.equal(topicEntities['topic-slug-1'].relateds)
    })
  })
})
