...
describe('Testing header actions:', () => {
  describe('Testing `setReadProgress` action creator:', () => {
    it('Should return percen ranges from 0 to 100', () => {
      ...
      expect(actions.setReadProgress(percntSmallThan0)).to.deep.equal(
        expectedState0
      )
      const percent = 20
      const expectedStatePercent = {
        type: types.SET_PROGRESS_PRECENTAGE,
        percent: 20,
      }
      expect(actions.setReadProgress(percent)).to.deep.equal(
        expectedStatePercent
      )
      const percntGreaterThan100 = 300
      const expectedState100 = {
        type: types.SET_PROGRESS_PRECENTAGE,
        percent: 100,
      }
      expect(actions.setReadProgress(percntGreaterThan100)).to.deep.equal(
        expectedState100
      )
    })
  })
  ...
  describe('Testing `setPageTitle` action creator:', () => {
    it('Should return articleId, pageTitle, and pageTopic', () => {
      ...
      const pageTitle = 'mockPageTitle'
      const pageTopic = 'mockPageTopic'
      const expectedState = {
      ...
      expect(
        actions.setPageTitle(articleId, pageTitle, pageTopic)
      ).to.deep.equal(expectedState)
      expect(actions.setPageTitle(articleId, pageTitle)).to.deep.equal(
        expectedStateNoTopic
      )
    })
  })
  ...
  describe('Testing `setBookmarksOfLongformArticle` action creator:', () => {
    it('Should return bookmarks', () => {
      ...
      expect(actions.setBookmarksOfLongformArticle(bookmarks)).to.deep.equal(
        expectedState
      )
    })
  })
